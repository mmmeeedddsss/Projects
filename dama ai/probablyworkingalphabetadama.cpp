...
pair< int,pair<int,int>  > evaluateF( int depth, int x, int y, int alpha, int beta )
{
		...
		mov = make_pair(x,y);
				...
				if( board[x][y] == 'W' || ( board[x][y] == 'w' && wPieceCount <= 3 ) )
					{
						...
						for( int i=y-1;i>=0; i--  ) // to top
						{
							...
							else if( flag == true )
							{
								...
								if( maxx >= beta ){ undo(x,y,x,i,c); bPieceCount++; board[x][pc]=t; return make_pair(maxx,mov);}} 
						...
						for( int i=x+1;i<=7; i++  ) // to right
						{
							...
							else if( flag == true )
							{
								...
								if( maxx >= beta ){undo(x,y,i,y,c); bPieceCount++; board[pc][y]=t; return make_pair(maxx,mov);}} 
								undo(x,y,i,y,c); bPieceCount++; board[pc][y]=t;
							}
						}
				...
				else if( board[x][y] == 'w' )
				{
								...
								{ 
									...
									if( maxx >= beta ){ undo(x,y,x-2,y,'w'); bPieceCount++; board[x-1][y]=t;  return make_pair(maxx,mov); }} 
								...
								{ 
									...
									if( maxx >= beta ){undo(x,y,x,y-2,'w'); bPieceCount++; board[x][y-1]=t;  return make_pair(maxx,mov);} } 
	...
	else
	{
		...
		mov = make_pair(x,y);
			...
			if( board[x][y] == 'B' || ( board[x][y] == 'w' && bPieceCount <= 3 ) )
					{
						char c = board[x][y];
						bool flag = false;
						int pc;
						for( int i=y-1;i>=0; i--  ) // to top
						{
							...
							else if( flag == true )
							{
								...
								if( alpha >= minn ){ undo(x,y,x,i,c); wPieceCount++; board[x][pc]=t; return make_pair(minn,mov);}} 
						...
						for( int i=x+1;i<=7; i++  ) // to right
						{
							...
							else if( flag == true )
							{
								...
								if( alpha >= minn ){ undo(x,y,i,y,c); wPieceCount++; board[pc][y]=t; return make_pair(minn,mov);}} 
			...
			else if( board[x][y] == 'b' )
			{
						...
						{ 
							...
							if( alpha >= minn ){ undo(x,y,x-2,y,'b'); wPieceCount++; board[x-1][y]=t;  return make_pair(minn,mov);} }
						...
						{ 
							...
							if( alpha >= minn ){ undo(x,y,x,y+2,'b'); wPieceCount++; board[x][y+1]=t; return make_pair(minn,mov);} }
			...
			for( int x=0; x<8; x++ )
				{
					...
					if( board[x][y] == 'W' || ( board[x][y] == 'w' && wPieceCount <= 3 ) )
					{
						...
						for( int i=y-1;i>=0; i--  ) // to top
						{
							...
							else if( flag == true )
							{
								...
								if( maxx >= beta ){undo(x,y,x,i,c); bPieceCount++; board[x][pc]=t; return maxx;}} 
								undo(x,y,x,i,c); bPieceCount++; board[x][pc]=t;
							}
						...
						for( int i=x+1;i<=7; i++  ) // to right
						{
							...
							else if( flag == true )
							{
								t=board[pc][y]; move(x,y,i,y,c); board[pc][y] = '*'; 
								pair< int,pair<int,int>  > pr = evaluateF(depth,i,y,maxx,beta);
								if(maxx < (temp=(pr.first+getPieceVal(t,i)+getPosVal(i,y)))){ maxx = temp; if( maxx >= beta ){undo(x,y,i,y,c); bPieceCount++;board[pc][y]=t; return maxx;}} 
								undo(x,y,i,y,c); bPieceCount++; board[pc][y]=t;
							}
							else if( !shouldEat )// go without eating anything
							{
						...
						for( int i=x-1;i>=0; i--  ) // to left
						{
							...
							else if( flag == true )
							{
								...
								if( maxx >= beta ){undo(x,y,i,y,c); bPieceCount++; board[pc][y]=t; return maxx;}} 
								undo(x,y,i,y,c); bPieceCount++; board[pc][y]=t;
							}
							else if( !shouldEat ) // go without eating anything
							{
					...
					else if( board[x][y] == 'w' ) // if using piece is just a normal one
					{
						...
						{ 
							...
							if( maxx >= beta ){  undo(x,y,x-2,y,'w'); bPieceCount++; board[x-1][y]=t; return maxx; }} 
							undo(x,y,x-2,y,'w'); bPieceCount++; board[x-1][y]=t; 
						}
						if( x+2<=7 && ( board[x+1][y] == 'b' || board[x+1][y] == 'B' ) && board[x+2][y] == '*' ) 
						{ 
						...
						{ 
							...
							if(maxx < temp){ maxx = temp; if( maxx >= beta ){  undo(x,y,x,y-2,'w'); bPieceCount++; board[x][y-1]=t; return maxx; }} 
							undo(x,y,x,y-2,'w'); bPieceCount++; board[x][y-1]=t; 
						}
					}
				}
					...
					if( board[x][y] == 'B' || ( bPieceCount <= 3  ) )
					{
						...
						for( int i=y-1;i>=0; i--  ) // to top
						{
							...
							else if( flag == true )
							{
								...
								if( alpha>=minn ){ undo(x,y,x,i,c); wPieceCount++; board[x][pc]=t; return minn; }} 
								undo(x,y,x,i,c); wPieceCount++; board[x][pc]=t;
							}
							else if( !shouldEat )// go without eating anything
							{
						...
						for( int i=x+1;i<=7; i++  ) // to right
						{
							...
							else if( flag == true )
							{
								...
								if( alpha>=minn ){ undo(x,y,i,y,c); wPieceCount++; board[pc][y]=t; return minn; }} 
								undo(x,y,i,y,c); wPieceCount++; board[pc][y]=t;
							}
							else if( !shouldEat )// go without eating anything
							{
					...
					else if( board[x][y] == 'b' ) // if using piece is just a normal one
					{
						...
						{ 
							...
							if(minn > (temp=(evaluateF(depth,x-2,y,alpha,minn).first-getPieceVal(t,x-1)-getPosVal(x-2,y))))
								{ minn = temp;  if( alpha>=minn ){ undo(x,y,x-2,y,'b'); board[x-1][y]=t; wPieceCount++; return minn; }} 
							undo(x,y,x-2,y,'b'); wPieceCount++; board[x-1][y]=t; 
						}
						if( x+2<=7 && ( board[x+1][y] == 'w' || board[x+1][y] == 'W' ) && board[x+2][y] == '*' ) 
						{ 
							...
							if(minn > (temp=(evaluateF(depth,x+2,y,alpha,minn).first-getPieceVal(t,x+1)-getPosVal(x+2,y))))
								{ 
									minn = temp;  
									if( alpha>=minn )
										{ undo(x,y,x+2,y,'b'); wPieceCount++; board[x+1][y]=t;  return minn; }
								} 
							undo(x,y,x+2,y,'b'); wPieceCount++; board[x+1][y]=t; 
						}
						if( y+2<=7 && ( board[x][y+1] == 'w' || board[x][y+1] == 'W' ) && board[x][y+2] == '*') 
						{ 
							...
							if(minn > temp){ minn = temp; if( alpha>=minn ){ undo(x,y,x,y+2,'b'); wPieceCount++; board[x][y+1]=t; return minn; }} 
							undo(x,y,x,y+2,'b'); wPieceCount++; board[x][y+1]=t; 
						}
					}
...
pair< pair<int,int> ,pair<int,int> > evaluater( int depth, int alpha, int beta ) // white trys to maximize , white minimize  
{
	if( depth == 0 || wPieceCount <= 0 || bPieceCount <= 0 )
	{
		...
		for( int x=0; x<8; x++ )
			for( int y=0; y<8; y++ )
			{
				if( board[x][y] == 'W' || wPieceCount <= 3 )
					val += points->damaGained;
				else if( board[x][y] == 'w' )
					val += points->pieceTaken;
				else if( board[x][y] == 'B' || bPieceCount <= 3 )
					val -= points->damaGained;
				else if( board[x][y] == 'b' )
					val -= points->pieceTaken;
			}
